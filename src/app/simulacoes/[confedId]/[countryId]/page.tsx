...
export default async function CountryPage({
  ...
  return (
    <div className="pt-20 min-h-screen max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            {country.divisions.map((div) => (
              <div key={div.id} className="glass rounded-xl p-5">
                ...
                <p className="text-xs text-muted">
                  Clubes nesta divisão:{" "}
                  {country.clubs.filter((c) => c.divisionId === div.id).length}
                </p>
              </div>
            ))}
          </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted mt-0.5">
                      ...
                      {league.status === "active" ? "Ativa" : league.status}
                    </p>
                  </div>
                </div>
              </Link>
      ...
      {/* Competições Simuladas deste País */}
          ...
          <div className="space-y-3">
            {competitions.map((comp) => (
              <Link
                ...
                className="glass rounded-xl p-4 hover:bg-card/60 transition-colors flex items-center gap-4 group"
              >
                {comp.logo ? (
                ...
                <span className="text-[10px] px-2 py-1 rounded-full bg-gold/10 text-gold font-semibold">
                  Simulado
                </span>
              </Link>
            ))}

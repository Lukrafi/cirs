...
import rankingData from "@/lib/concacaf-power-ranking.json";

export const dynamic = "force-dynamic";

export const metadata = {
  title: "Power Ranking CONCACAF — CIRS",
  description: "Ranking de força das ligas nacionais da CONCACAF — América do Norte, Central e Caribe.",
};

const CONF_COLOR = "#a855f7";
...
function ratingTier(rating: number): string {
  if (rating >= 800) return "bg-purple-500/10 text-purple-300";
  if (rating >= 650) return "bg-purple-500/10 text-purple-400";
  if (rating >= 500) return "bg-blue-500/10 text-blue-400";
...
export default function ConcacafPowerRankingPage() {
  return (
    <div className="pt-20 min-h-screen max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <h1 className="text-4xl font-black mb-2"><span className="gold-text">Power Ranking da CONCACAF</span></h1>
      <p className="text-muted mb-8 max-w-3xl">
        Ranking de força das ligas nacionais da América do Norte, Central e Caribe. Ratings de 0 a 1000 baseados em
      ...
      <div className="flex items-center gap-2 mb-8 flex-wrap">
        ...
        <span className="text-xs px-3 py-1.5 rounded-lg" style={{ background: `${CONF_COLOR}22`, color: CONF_COLOR }}>CONCACAF</span>
      ...
      {topLeague && (
        <div className="glass rounded-2xl p-6 mb-10 flex items-center justify-between">
          <div>
            <div className="text-xs uppercase tracking-wider text-muted">Liga #1 da CONCACAF</div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
        <div className="glass rounded-xl p-4 text-center"><div className="text-2xl font-bold gold-text">{entries.length}</div><div className="text-xs text-muted uppercase mt-1">Ligas Rankeadas</div></div>
        <div className="glass rounded-xl p-4 text-center"><div className="text-2xl font-bold gold-text">{firstDiv.length}</div><div className="text-xs text-muted uppercase mt-1">1ª Divisão</div></div>
        <div className="glass rounded-xl p-4 text-center"><div className="text-2xl font-bold gold-text">{secondDiv.length}</div><div className="text-xs text-muted uppercase mt-1">2ª Divisão</div></div>
  ...
  return (
    <section className="mb-12">
      <div className="flex items-end gap-3 mb-4">
        ...
        <h2 className="text-xl font-bold"><span style={{ color }}>{title}</span><span className="text-muted text-sm ml-2 font-normal">{entries.length} ligas</span></h2>
      </div>
      <div className="glass rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-border"><tr className="text-left text-muted text-xs uppercase tracking-wider">
              <th className="p-3 text-center w-12">#</th><th className="p-3 text-center w-10">Bandeira</th><th className="p-3">Liga</th><th className="p-3">País</th><th className="p-3 text-center w-28">Rating</th><th className="p-3 text-center w-40">Barra</th>
            </tr></thead>
            <tbody>
              {entries.map((e, idx) => (
                <tr key={`${e.league}-${e.country}-${idx}`} className="border-b border-border last:border-0 hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-3 text-center"><span className={`px-2 py-0.5 rounded-full text-xs font-bold ${ratingTier(e.rating)}`}>{e.rating}</span></td>
                  <td className="p-3"><div className="h-1.5 bg-blue-deep rounded-full overflow-hidden"><div className="h-full rounded-full transition-all" style={{ width: `${(e.rating / 1000) * 100}%`, background: `linear-gradient(90deg, ${color}, ${color}88)` }} /></div></td>
                </tr>
              ))}
            </tbody>
          </table>

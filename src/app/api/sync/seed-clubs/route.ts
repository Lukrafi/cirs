  ...
  try {
    ...
    for (const cData of confedData.countries) {
      ...
      for (const compData of cData.competitions) {
        ...
        let competition = await prisma.competition.findFirst({ where: { name: compData.name, seasonId: null } });
        if (!competition) {
          competition = await prisma.competition.create({
            data: { name: compData.name, type: compData.type, seasonId: null, numTeams: compData.teams.length, numTurns: 2, format: "round-robin", isKnockout: false },
          });
          competitionsCreated++;
        }

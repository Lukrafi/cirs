...
export type CafCompetitionType = "caf-champions-league" | "caf-confederations-cup";

export type CafPhase =
  | "preliminary-1"
  | "preliminary-2"
  | "groups"
  | "quarterfinals"
...
export const CAF_COMPETITIONS: Record<CafCompetitionType, {
  name: string;
  shortName: string;
  logo: string;
  color: string;
  ...
  "caf-champions-league": {
    ...
    logo: "",
    color: "#22c55e",
    description: "O principal torneio de clubes do futebol africano.",
    groupStageTeams: 16,
    groupStageGroups: 4,
    teamsPerGroup: 4,
    qualifyPerGroup: 2,
  ...
  "caf-confederations-cup": {
    ...
    logo: "",
    color: "#f59e0b",
    description: "O segundo torneio mais importante de clubes do continente africano.",
    groupStageTeams: 16,
    groupStageGroups: 4,
    teamsPerGroup: 4,
    qualifyPerGroup: 2,
...
export const PHASE_LABELS: Record<CafPhase, string> = {
  "preliminary-1": "Primeira Pré-Eliminatória",
  "preliminary-2": "Segunda Pré-Eliminatória",
...
export async function createCafCompetition(
  ...
  const competition = await prisma.competition.create({
    data: {
      ...
      isKnockout: false,
      format: "groups",
      numTeams: preliminaryRound1Teams.length,
      hasExtraTime: true,
      hasPenalties: true,
      pointsPerWin: 3,
      pointsPerDraw: 1,
  ...
  const p1Group = await prisma.group.create({
    data: { name: "Primeira Pré-Eliminatória", competitionId: competition.id },
  });

  const p1Fixtures = generateTwoLegKnockoutFixtures(preliminaryRound1Teams, startDate, 7, "P1");
  for (const m of p1Fixtures) {
...
export async function advanceToPreliminaryRound2(
  ...
  const p2Group = await prisma.group.create({
    data: { name: "Segunda Pré-Eliminatória", competitionId },
  });

  const p2Fixtures = generateTwoLegKnockoutFixtures(round1Winners, startDate, 7, "P2");
  for (const m of p2Fixtures) {
    await prisma.match.create({
      data: {
        homeTeamId: m.homeId, awayTeamId: m.awayId,
        groupId: p2Group.id,
...
export async function generateGroupStage(
  ...
  const numGroups = config.groupStageGroups;
  const perGroup = config.teamsPerGroup;
  ...
  for (let g = 0; g < numGroups; g++) {
    ...
    const group = await prisma.group.create({
      data: { name: `Grupo ${String.fromCharCode(65 + g)}`, competitionId },
    });
    ...
    const groupFixtures = generateRoundrobinFixtures(groupTeams, 2, new Date(startDate.getTime() + g * 86400000), 7);
...
export async function simulatePreliminaryRound(
  competitionId: string,
  roundLabel: string
...
export async function simulateGroupPhase(
  ...
  const matches = await prisma.match.findMany({
    where: {
      group: { competitionId, name: { startsWith: "Grupo" } },
      status: "scheduled",
      isKnockout: false,
    },
    include: { group: { include: { competition: true } } },
...
export async function generateQuarterfinalsFromGroups(
  competitionId: string,
  startDate: Date = new Date()
  ...
  for (const g of groups) {
    for (let i = 0; i < Math.min(config.qualifyPerGroup, g.standings.length); i++) {
      if (g.standings[i].clubId) qualified.push(g.standings[i].clubId!);
    }
  }

  const seeded = [
    ...qualified.filter((_, i) => i % 2 === 0),
    ...qualified.filter((_, i) => i % 2 === 1),
  ];

  let created = 0;
  const qfGroup = await prisma.group.create({
    data: { name: "Quartas de Final", competitionId },
  });

  const qfFixtures = generateTwoLegKnockoutFixtures(seeded, startDate, 7, "QF");
  for (const m of qfFixtures) {
    await prisma.match.create({
      data: {
        homeTeamId: m.homeId, awayTeamId: m.awayId,
        groupId: qfGroup.id,
  ...
  return { created, qualifiedTeamIds: seeded };
}
...
export async function simulateCafPhase(
  competitionId: string,
  phase: string,
  roundNumber?: string
): Promise<{ simulated: number; results: SimulationResult[]; winners?: string[] }> {
  switch (phase) {
    case "preliminary-1":
    case "P1":
      return await simulatePreliminaryRound(competitionId, "P1");

    case "preliminary-2":
    case "P2":
      return await simulatePreliminaryRound(competitionId, "P2");
